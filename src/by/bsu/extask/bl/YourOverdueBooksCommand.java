...
public class YourOverdueBooksCommand implements Command{
...
private static String loginParam = "login";
	...
	public String execute(InDataWrapper inData) throws DataException {
		...
		int result = 0;
		String page = null;
		DeliveryList yourOverdueBooks;
		
		try{
			
			//result = ReaderDAO.oneReader(number, name, surname, patronymic, adress, phone);
			yourOverdueBooks = YourOverdueBooksDAO.yourOverdueBooks(login);
			...
			if (yourOverdueBooks == null){
				throw new DataException("Ошибка работы с источником данных. Нет книг.");
			}
			
			inData.setRequestAttribute("yourOverdueBooks", yourOverdueBooks);
	
		}
		catch(DAOException e) {
			throw new DataException("Ошибка работы с источником данных in YourOverdueBooksCommand");
			
		}
		
		return "/WEB-INF/jsp/your_overdue_books.jsp";
